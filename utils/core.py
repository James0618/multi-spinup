...
def cnn(args):
    ...
    cnn_layer = nn.Sequential(
        nn.Conv2d(in_channels=image_shape[0], out_channels=32, kernel_size=4, stride=2),
        nn.ReLU(),
...
class CNNActor(Actor):
    ...
    def _distribution(self, obs):
        if len(obs.shape) == 3:
            obs = obs.unsqueeze(0)
        n_batch = obs.shape[0]
        feature = self.feature_net(obs).reshape(n_batch, -1).squeeze()
        logits = self.logits_net(feature)
        return Categorical(logits=logits)
...
class CNNCritic(nn.Module):
    ...
    def forward(self, obs):
        ...
        n_batch = obs.shape[0]
        feature = self.feature_net(obs).reshape(n_batch, -1).squeeze()
        return torch.squeeze(self.v_net(feature), -1)  # Critical to ensure v has right shape.
