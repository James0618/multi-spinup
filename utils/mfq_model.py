...
class MFQ(nn.Module):
    def __init__(self, args):
        ...
        if self.args.action_net:
            ...
            self.q_net = nn.Sequential(
                nn.Linear(self.obs_shape + args.n_actions, args.hidden_dim),
                nn.ReLU(),
    ...
    def forward(self, obs, mean_actions):
        ...
        if self.args.action_net:
            ...
            feature = torch.cat((obs, mean_actions), dim=-1)
...
class Policy:
    ...
    def choose_action(self, observations, mean_actions, epsilon=1.0, temperature=1.0):
        actions = {}
        ...
        act_tensor = torch.from_numpy(mean_actions).type(torch.float).cuda()
        act_tensor = act_tensor.expand(torch.Size([n_agent, *act_tensor.shape]))

        for key in observations.keys():
            obs_tensor[self.agents.index(key)] = observations[key].cuda()

        q = self.model(obs_tensor, act_tensor)
